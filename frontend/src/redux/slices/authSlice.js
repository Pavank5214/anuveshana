...
// Check for an existing guest ID in the localStorage or create a new one
...
export const googleLogin = createAsyncThunk("auth/googleLogin", async ({ accessToken, action }, { rejectWithValue }) => {
    try {
        const response = await axios.post(
            `${import.meta.env.VITE_BACKEND_URL}/api/users/google-login`, { accessToken, action });
        localStorage.setItem("userInfo", JSON.stringify(response.data.user));
        localStorage.setItem("userToken", response.data.token);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(loginUser.fulfilled, (state, action) => {
                state.loading = false;
                state.user = action.payload; // store the user
                state.error = null;
            })
