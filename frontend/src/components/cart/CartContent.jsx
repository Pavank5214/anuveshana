...
const CartContent = ({ cart, userId, guestId }) => {
  ...
  const handleQtyChange = (productId, delta, quantity, size, color, personalization) => {
    const newQty = quantity + delta;
    if (newQty >= 1) {
      dispatch(updateCartItemQuantity({ productId, quantity: newQty, guestId, userId, size, color, personalization }));
    }
  };

  const handleRemove = (productId, size, color, personalization) => {
    dispatch(removeFromCart({ productId, guestId, userId, size, color, personalization }));
  };
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                ...
                {product.personalization && Object.entries(product.personalization).map(([k, v]) => (
                  <span key={k} className="flex items-center gap-1 text-[10px] bg-white/5 border border-white/10 text-slate-400 px-2 py-0.5 rounded-full text-nowrap">
                    {v && typeof v === 'string' && v.startsWith('#') && (
                      <span className="w-2.5 h-2.5 rounded-full border border-white/20 inline-block" style={{ backgroundColor: v.toLowerCase() }} />
                    )}
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center border border-white/15 rounded-lg overflow-hidden bg-white/5 h-7">
                  <button
                    onClick={() => handleQtyChange(product.productId, -1, product.quantity, product.size, product.textColor, product.personalization)}
                ...
                {/* Delete */}
                <button
                  onClick={() => handleRemove(product.productId, product.size, product.textColor, product.personalization)}
