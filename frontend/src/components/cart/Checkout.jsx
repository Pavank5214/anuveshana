...
const Checkout = () => {
    ...
    const handleFinalizeCheckout = async(checkoutId)=>{
        try {
            ...
            axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/checkout/${checkoutId}/finalize`, 
                {},
            {
                headers: {
                     Authorization : `Bearer ${localStorage.getItem("userToken")}`
    ...
    <div className="grid grid-col-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto py-10 px-6 tracking-tighter">
        ...
        <div className="bg-white rounded-lg p-6">
            ...
            <form onSubmit={handleCreateCheckout}>
                ...
                <h3 className="tect-lg mb-4">Delivery</h3>
                ...
                {cart.products.map((product, index)=>(
                    <div key={index} className="flex items-start justify-between py-2 border-b">
                        <div className="flex items-start">
                            <img src={product.image} alt={product.name} className="w-20 h-24 cover mr-4"/>
                            <div>
