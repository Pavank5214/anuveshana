...
const ProductDetails = ({ productId }) => {
  ...
  const handleAddToCart = () => {
    const hasDynamic = selectedProduct.personalizationFields?.length > 0;

    if (hasDynamic) {
      const missingField = selectedProduct.personalizationFields.find(f => f.required && !personalization[f.label]);
      if (missingField) {
        setShowErrors(true);
        toast.error(`Please select ${missingField.label}`, { duration: 1800 });
        return;
      }
    }
    ...
    dispatch(addToCart({
      productId: productFetchId, quantity,
      size: null,
      custName: null,
      textColor: null,
      baseColor: null,
      personalization: hasDynamic ? personalization : null,
      guestId, userId: user?._id,
    }))
  ...
  const handleOrderNow = () => {
    ...
    dispatch(addToCart({
      ...
      .then(() => {
        if (!user) {
          toast.error("Please login to complete your order", { duration: 2000 });
  ...
  return (
    <div className="bg-[#0B0F19] min-h-screen  pt-24 pb-24 sm:pb-16 mt-7">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1}
            ...
            <motion.h1 variants={fadeUp} initial="hidden" animate="visible" custom={0}
              className="text-2xl sm:text-3xl font-black tracking-tighter leading-[1.1] text-white">
            ...
            {/* Main Image */}
            <div className="relative w-full max-w-md mx-auto aspect-square sm:aspect-[1/1] lg:aspect-[4/5] rounded-xl overflow-hidden border border-white/10 shadow-2xl group">
              <div className="absolute -inset-1 rounded-xl bg-gradient-to-br from-[#ff6200]/20 to-orange-400/5 blur-xl z-0" />
            ...
            <div className="flex gap-2.5 overflow-x-auto pb-1 scrollbar-hide justify-center">
              {selectedProduct?.images?.map((image, index) => (
                <button key={index} onClick={() => setMainImage(image.url)}
                  className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all duration-300 ${mainImage === image.url
            ...
            {/* ── Personalization ── */}
            {selectedProduct.personalizationFields && selectedProduct.personalizationFields.length > 0 && (
              <DynamicPersonalization
                fields={selectedProduct.personalizationFields}
                values={personalization}
                onChange={(label, value) => setPersonalization(prev => ({ ...prev, [label]: value }))}
                showErrors={showErrors}
              />
            )}
            {/* Removed legacy fallback block */}
          ...
          <div className="flex flex-col">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={4}
              className="text-slate-400 leading-relaxed text-base mb-8 whitespace-pre-wrap">
