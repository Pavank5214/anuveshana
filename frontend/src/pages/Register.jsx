...
import { Loader2, User, Mail, Lock, Zap, ArrowRight } from "lucide-react";
...
const Register = () => {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      dispatch(googleLogin({ accessToken: tokenResponse.access_token, action: "register" }));
    },
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] flex items-center justify-center px-4 mt-5 py-8 sm:py-16">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-10 shadow-2xl">
          <div className="flex flex-col items-center mb-6 sm:mb-8">
            ...
            <h1 className="text-2xl sm:text-3xl font-black tracking-tighter text-white text-center">
              Create <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff6200] to-orange-400">Account</span>
            </h1>
            <p className="text-slate-500 text-xs sm:text-sm mt-2 text-center">Fill in your details to get started</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Full Name</label>
            ...
            <div>
              <label className="block text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Password</label>
              <div className="relative">
          ...
          <p className="mt-6 text-center text-slate-500 text-xs sm:text-sm">
            ...
            <Link to={`/login?redirect=${encodeURIComponent(from)}`} className="text-orange-400 hover:text-orange-300 font-semibold transition-colors">
              Login here
            </Link>
          </p>
        </div>
